...
class Game extends CI_Controller
{
	function index($reload = false)
	{
		if(! $this->player->isvalid())
			return $this->start();
		$this->load->model('zones/' . $this->session->userdata('zone'), 'zone');
		$this->load->model('areas/' . $this->session->userdata('area'), 'area');
		...
		foreach($inventory as $k => $v)
		{
			...
			$items[] = array($this->$k->name, $this->$k->description, $this->$k->image);
		}
